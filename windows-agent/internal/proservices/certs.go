...
func newTLSCertificates(destDir string) (c agentCerts, err error) {
	...
	if err != nil {
		return agentCerts{}, fmt.Errorf("failed to generate serial number for the CA cert: %v", err)
	}
	...
	return agentCerts{rootCA: rootCert, serverCert: *serverCert}, nil
}

// agentTLSConfig returns a TLS config for servers that require and verify client certificates.
...
// agentCerts conveniently holds the root CA and server certificates to make it easy to create a TLS config.
