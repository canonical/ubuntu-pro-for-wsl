...
function Edit-AppxManifest {
    ...
    if ($Mode -eq "end_to_end_tests") {
        # Remove the auto-update on launch setting that's preventing CI to start the process.
        $launchSetting = $doc.Root.Element($ns + "UpdateSettings").Element("OnLaunch")
        if($null -ne $launchSetting) {
            $launchSetting.Remove()
        }
    }
